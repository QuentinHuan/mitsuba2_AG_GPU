...
MTS_VARIANT Shape<Float, Spectrum>::Shape(const Properties &props)
    : m_id(props.id()) {
    ...
    for (auto &[name, obj] : props.objects(false)) {
        ...
        if (emitter) {
            if (m_emitter)
                Throw("Only a single Emitter child object can be specified per "
                      "shape.");
            m_emitter = emitter;
        } else if (sensor) {
            if (m_sensor)
                Throw("Only a single Sensor child object can be specified per "
                      "shape.");
            m_sensor = sensor;
        } else if (bsdf) {
            if (m_bsdf)
                Throw("Only a single BSDF child object can be specified per "
                      "shape.");
            m_bsdf = bsdf;
        } else if (medium) {
            if (name == "interior") {
    ...
    m_caustic_caster = props.bool_("caustic_caster", false);
    m_fermat_shape     = props.bool_("fermat_shape", false);
}
...
Shape<Float, Spectrum>::sample_position(Float /*time*/,
                                        const Point2f & /*sample*/,
                                        Mask /*active*/) const {
    NotImplementedError("sample_position");
}

MTS_VARIANT Float Shape<Float, Spectrum>::pdf_position(
...
template <typename Float, typename Spectrum>
void embree_intersect_scalar(int *valid, void *geometryUserPtr,
                             unsigned int geomID, unsigned int instID,
                             RTCRay *rtc_ray, RTCHit *rtc_hit) {
    MTS_IMPORT_TYPES(Shape)
...
template <typename Float, typename Spectrum>
void embree_intersect_packet(int *valid, void *geometryUserPtr,
                             unsigned int geomID, unsigned int instID,
                             RTCRayW *rays, RTCHitW *hits) {
    MTS_IMPORT_TYPES(Shape)
...
MTS_VARIANT RTCGeometry
Shape<Float, Spectrum>::embree_geometry(RTCDevice device) {
    if constexpr (!is_cuda_array_v<Float>) {
        RTCGeometry geom = rtcNewGeometry(device, RTC_GEOMETRY_TYPE_USER);
        rtcSetGeometryUserPrimitiveCount(geom, 1);
        rtcSetGeometryUserData(geom, (void *) this);
        rtcSetGeometryBoundsFunction(geom, embree_bbox<Float, Spectrum>,
                                     nullptr);
        rtcSetGeometryIntersectFunction(geom,
                                        embree_intersect<Float, Spectrum>);
...
#if defined(MTS_ENABLE_OPTIX)
static const uint32_t optix_geometry_flags[1] = {
...
void Shape<Float, Spectrum>::optix_fill_hitgroup_records(
    ...
    rt_check(optixSbtRecordPackHeader(program_groups[program_group_idx],
                                      &hitgroup_records.back()));
}

MTS_VARIANT void Shape<Float, Spectrum>::optix_prepare_ias(
    const OptixDeviceContext & /*context*/,
    std::vector<OptixInstance> & /*instances*/, uint32_t /*instance_id*/,
    const ScalarTransform4f & /*transf*/) {
    NotImplementedError("optix_prepare_ias");
}

MTS_VARIANT void
Shape<Float, Spectrum>::optix_build_input(OptixBuildInput &build_input) const {
...
MTS_VARIANT Float Shape<Float, Spectrum>::pdf_direction(
    const Interaction3f & /*it*/, const DirectionSample3f &ds,
    Mask active) const {
    MTS_MASK_ARGUMENT(active);
...
MTS_VARIANT std::pair<typename Shape<Float, Spectrum>::Vector3f,
                      typename Shape<Float, Spectrum>::Vector3f>
...
MTS_VARIANT Float Shape<Float, Spectrum>::eval_texture(int index, Point2f p,
                                                       Mask active) const {
    NotImplementedError("eval_texture");
}

MTS_VARIANT void Shape<Float, Spectrum>::fill_curvature(Point2f p, Float *r,
                                                        Mask active) const {
    NotImplementedError("fill_curvature");
}
MTS_VARIANT typename Shape<Float, Spectrum>::Matrix2f
Shape<Float, Spectrum>::curvature_matrix(const SurfaceInteraction3f &si,
                                         Mask active) const {
    NotImplementedError("curvature_matrix");
}

MTS_VARIANT int Shape<Float, Spectrum>::get_pair_id(Mask /*active*/) const {
    NotImplementedError("get_pair_id");
}

MTS_VARIANT int Shape<Float, Spectrum>::get_self_id(Mask /*active*/) const {
    NotImplementedError("get_self_id");
}
...
Shape<Float, Spectrum>::compute_surface_interaction(
    const Ray3f & /*ray*/, PreliminaryIntersection3f /*pi*/,
    HitComputeFlags /*flags*/, Mask /*active*/) const {
    NotImplementedError("compute_surface_interaction");
}

MTS_VARIANT typename Shape<Float, Spectrum>::SurfaceInteraction3f
Shape<Float, Spectrum>::ray_intersect(const Ray3f &ray, HitComputeFlags flags,
                                      Mask active) const {
    MTS_MASK_ARGUMENT(active);
...
MTS_VARIANT Float Shape<Float, Spectrum>::eval_attribute_1(
    const std::string & /*name*/, const SurfaceInteraction3f & /*si*/,
    Mask /*active*/) const {
...
MTS_VARIANT typename Shape<Float, Spectrum>::ScalarBoundingBox3f
Shape<Float, Spectrum>::bbox(ScalarIndex index,
...
MTS_VARIANT
void Shape<Float, Spectrum>::parameters_changed(
