...
public:
    ...
    FermatPathIntegrator(const Properties &props) : Base(props) {
        fnee_config.max_bernouilli_trial =
            props.int_("max_bernouilli_trial", 20);
        fnee_config.solution_identical_threshold =
            props.float_("solution_identical_threshold", 1e-5f);
        fnee_config.solver_gradient_threshold =
            props.float_("solver_gradient_threshold", 1e-4f);
        fnee_config.alpha         = props.float_("alpha", 0.5f);
        fnee_config.caustic_bleed = props.bool_("caustic_bleed", false);

        fermatNEE = new FermatNEE(fnee_config);
    }
    ...
    std::string to_string() const override {
        return tfm::format("FermatPathIntegrator[\n"
                           "  max_depth = %i,\n"
                           "  rr_depth = %i\n"
                           "]",
...
MTS_IMPLEMENT_CLASS_VARIANT(FermatPathIntegrator, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(FermatPathIntegrator, "Fermat Path Tracer integrator");
